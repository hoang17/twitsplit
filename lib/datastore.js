...
function getLikeId(userIP, questionId){
  return userIP.replace(/\./g,'_') + '_' + questionId
}

async function isLiked(userIP, questionId){
  var id = getLikeId(userIP, questionId)
  var doc = await fsLikes.get(id)
  return doc.exists
}

function like(userIP, questionId, liked){
  var id = getLikeId(userIP, questionId)
  if (liked)
